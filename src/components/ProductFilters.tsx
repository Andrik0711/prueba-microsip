import { Box, FormControlLabel, Switch, TextField, Button, Dialog, DialogTitle, DialogContent, DialogActions, Slider, Typography } from '@mui/material';
...
export default function ProductFilters({ search, setSearch, showOnlyModified, setShowOnlyModified, products, minPrice, maxPrice, setMinPrice, setMaxPrice }: Props) {
  ...
  return (
    <Box display="flex" alignItems="center" mb={2} gap={2}>
      ...
      <Dialog open={filtersOpen} onClose={() => setFiltersOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          <Box px={1}>
            <Slider
              ...
              marks={[{ value: priceRange.min, label: `$${priceRange.min.toFixed(2)}` }, { value: priceRange.max, label: `$${priceRange.max.toFixed(2)}` }]}
              sx={{ mb: 2 }}
            />
        ...
        <DialogActions>
          <Button onClick={() => {
            setMinPrice(null); setMaxPrice(null); setSliderValue([priceRange.min, priceRange.max]);
          }}>Limpiar</Button>
          <Button onClick={() => setFiltersOpen(false)} variant="contained">Cerrar</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
